...
const Admin = () => {
  ...
  const handleStreamControl = async (action) => {
    ...
    try {
      ...
      // Tự động bật camera khi stream bắt đầu
      if (action === 'START' && (response.data?.isLive || response.data?.live)) {
        console.log('🎥 Auto-starting camera... (isLive:', response.data?.isLive, 'live:', response.data?.live, ')');
        setTimeout(() => {
          startCamera();
        }, 1000); // Delay 1 giây để đảm bảo UI đã cập nhật
      }
      
      // Tự động tắt camera khi stream dừng
      if (action === 'STOP') {
